...
bool TGA::loadFromFile(const char *pFilename, unsigned int *pWidth,
                       unsigned int *pHeight, unsigned int *pPixelDepth,
  ...
  *ppPixelData = 0;
  ...
  // to be careful of the sign e.g. char c = 0x80 then need to cast to unsigned
  // char before I do anything with it So put horrible casts here since what we
  // what to read are UNSIGNED 8 bit values
  //
  TGAHeader l_tgaHdr;
...
bool TGA::saveToFile(const char *pFilename, const unsigned int width,
                     const unsigned int height, const unsigned int pixelDepth,
...
#define LSB_MSB(val)                                                           \
  (unsigned char)((val) & 0xff), (unsigned char)(((val) & 0xff00) >> 8)
  TGAHeader l_tgaHdr = {
      0, // ID Len
      0, // No Color Map
      (unsigned char)l_imageType,
      (unsigned char)((0) & 0xff),
      (unsigned char)(((0) & 0xff00) >> 8), // Color map index
      (unsigned char)((0) & 0xff),
      (unsigned char)(((0) & 0xff00) >> 8), // Color map len
      0,                                    // Color map size
      (unsigned char)((0) & 0xff),
      (unsigned char)(((0) & 0xff00) >> 8), // X origin
      (unsigned char)((0) & 0xff),
      (unsigned char)(((0) & 0xff00) >> 8), // Y origin
