...
//
// YES, this is a mess. Some of the functions are useful, but no one else can
// find them :P
//
namespace Util {
...
inline bool startswith(const std::string &s, const std::string &e) {
...
inline std::string shorten(const std::string &s, unsigned int len) {
  return (s.size() >= len) ? s.substr(0, s.size() - len) : "";
}

///////////////////////////////////////////////////////////////////////

inline std::string shortenFront(const std::string &s, unsigned int len) {
...
template <typename NumType> NumType strToNum(const std::string &rS, int base) {
  NumType value = 0;
...
template <typename NumType> NumType strToNum(const std::string &rS) {
  return strToNum<NumType>(rS, (Util::startswith(rS, "0x") ? 16 : 10));
}

inline long decStrToLong(const std::string &rS) {
  return strToNum<long>(rS, 10);
}
inline long hexStrToLong(const std::string &rS) {
  return strToNum<long>(rS, 16);
}

inline unsigned long decStrToULong(const std::string &rS) {
  return strToNum<unsigned long>(rS, 10);
}
inline unsigned long hexStrToULong(const std::string &rS) {
  return strToNum<unsigned long>(rS, 16);
}
...
inline int decStrToShort(const std::string &rS) {
  return strToNum<short>(rS, 10);
}
inline int hexStrToShort(const std::string &rS) {
  return strToNum<short>(rS, 16);
}

inline unsigned int decStrToUInt(const std::string &rS) {
  return strToNum<unsigned int>(rS, 10);
}
inline unsigned int hexStrToUInt(const std::string &rS) {
  return strToNum<unsigned int>(rS, 16);
}

inline float decStrToFloat(const std::string &rS) {
  return strToNum<float>(rS, 10);
}
inline float decStrToDouble(const std::string &rS) {
  return strToNum<double>(rS, 10);
}

///////////////////////////////////////////////////////////////////////

inline std::string rtrim(const std::string &rS,
                         const char *pTrimChars = WS_CHARS) {
  std::string::size_type l_nonWS = rS.find_last_not_of(pTrimChars);
  if (l_nonWS == std::string::npos) {
    return std::string();
  } else if ((l_nonWS + 1) < rS.size()) {
    return std::string(rS, 0, l_nonWS + 1);
  }
  return rS;
}

inline std::string ltrim(const std::string &rS,
                         const char *pTrimChars = WS_CHARS) {
...
inline std::string trim(const std::string &rS,
                        const char *pTrimChars = WS_CHARS) {
...
//
// Return the input string left/right padded to the length (or unchanged if
// longer)
//
inline std::string lpad(unsigned int padLen, const std::string &s,
                        char padChr = ' ') {
  return (s.size() < padLen) ? std::string(padLen - s.size(), padChr) + s : s;
}
inline std::string rpad(unsigned int padLen, const std::string &s,
                        char padChr = ' ') {
  return (s.size() < padLen) ? s + std::string(padLen - s.size(), padChr) : s;
}
// Pads the input to the new length with padding at start/end
inline std::string cpad(unsigned int padLen, const std::string &s,
                        char padChr = ' ') {
  const int rpadLen =
      (int(padLen) - int(s.size())) / 2 + (int(padLen) - int(s.size())) % 2;
...
inline std::string removeUpToLast(const std::string &rS, char c) {
...
inline std::string removeFromLast(const std::string &rS, char c) {
  std::string::size_type l_cutPos = rS.rfind(c);
  return (l_cutPos != std::string::npos) ? std::string(rS).erase(l_cutPos) : rS;
}

inline std::string removeUpToFirst(const std::string &rS, char c) {
...
inline std::string removeFromFirst(const std::string &rS, char c) {
  std::string::size_type l_cutPos = rS.find(c);
  return (l_cutPos != std::string::npos) ? std::string(rS).erase(l_cutPos) : rS;
}
...
//
inline std::string makeSingleLine(const std::string &str) {
  return replaceAll(str, "\n", " ");
}

///////////////////////////////////////////////////////////////////////

inline int splitString(const std::string &rString, const std::string &rDelim,
                       std::vector<std::string> &rSubStrs,
                       bool allowBlank = false, bool doTrim = true) {
...
inline void tokenizeStr(const std::string &str,
                        std::vector<std::string> &rTokens,
                        const std::string &delimiters = WS_CHARS) {
...
std::string numToStr(NumType aValue, int aBase = 10, int aWidth = 0,
                     int aPrecision = -1) {
  std::ostringstream oss;
  // Don't want to use fixed if not given precision
  if (aPrecision != -1) {
    oss << std::fixed << std::setprecision(aPrecision);
  }
  ...
  oss << std::setfill('0') << std::setw(aWidth) << std::setbase(aBase)
      << aValue;

  return oss.str();
}

inline std::string ulongToDecStr(unsigned long value, int w = 0) {
  return numToStr<unsigned long>(value, 10, w);
}
inline std::string ushortToDecStr(unsigned short value, int w = 0) {
  return numToStr<unsigned short>(value, 10, w);
}

inline std::string longToDecStr(long value, int w = 0) {
  return numToStr<long>(value, 10, w);
}
inline std::string shortToDecStr(short value, int w = 0) {
  return numToStr<short>(value, 10, w);
}

inline std::string ulongToHexStr(unsigned long value, int w = 0) {
  return numToStr<unsigned long>(value, 16, w);
}
inline std::string ushortToHexStr(unsigned short value, int w = 0) {
  return numToStr<unsigned short>(value, 16, w);
}

inline std::string longToHexStr(long value, int w = 0) {
  return numToStr<long>(value, 16, w);
}
inline std::string shortToHexStr(short value, int w = 0) {
  return numToStr<float>(value, 16, w);
}

inline std::string intToDecStr(int value, int w = 0) {
  return numToStr<int>(value, 10, w);
}
inline std::string uintToDecStr(unsigned int value, int w = 0) {
  return numToStr<unsigned int>(value, 10, w);
}

inline std::string intToHexStr(int value, int w = 0) {
  return numToStr<int>(value, 16, w);
}
inline std::string uintToHexStr(unsigned int value, int w = 0) {
  return numToStr<unsigned int>(value, 16, w);
}

inline std::string floatToDecStr(float value, int w = 0) {
  return numToStr<float>(value, 10, w);
}

// Call numToStr using short to print char as number
inline std::string ucharToDecStr(unsigned char value, int w = 0) {
  return numToStr<unsigned short>(value, 10, w);
}
inline std::string charToDecStr(char value, int w = 0) {
  return numToStr<short>(value, 10, w);
}

inline std::string ucharToHexStr(unsigned char value, int w = 0) {
  return numToStr<unsigned short>(value, 16, w);
}
inline std::string charToHexStr(char value, int w = 0) {
  return numToStr<short>(value, 16, w);
}

///////////////////////////////////////////////////////////////////////

template <typename NumType>
int getNumbers(const std::vector<std::string> &valueStrs,
               std::vector<NumType> &values) {
  for (std::vector<std::string>::const_iterator l_itr = valueStrs.begin();
       l_itr != valueStrs.end(); ++l_itr) {
...
template <typename NumType>
int getNumbers(const std::string &rString, const std::string &rDelim,
               std::vector<NumType> &rValues) {
  std::vector<std::string> l_valueStrs;
  int l_numValues =
      Util::splitString(rString, rDelim, l_valueStrs, false, false);
