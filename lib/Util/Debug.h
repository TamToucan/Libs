  ...
  int debugFlagsFromString(const std::string& s) {
    ...
    for (std::vector<std::string>::const_iterator l_itr = l_flagStrs.begin();
      ...
      if (*l_itr == "ERROR")
        ...
        l_debugFlags |= Util::Debug::WARNING_ON;
      else if (*l_itr == "INFO")
        l_debugFlags |= Util::Debug::INFO_ON;
      else if (*l_itr == "DEBUG")
        l_debugFlags |= Util::Debug::DEBUG_ON;
      else if (*l_itr == "STACK")
        l_debugFlags |= Util::Debug::LOGENTRY_ON;
      else if (*l_itr == "ALL")
        l_debugFlags |= Util::Debug::ALL_ON;
    }
    return l_debugFlags;
  }
