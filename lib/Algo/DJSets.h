...
#ifndef ALGO_DJ_SETS_H
#define ALGO_DJ_SETS_H
...
public:
    ...
    // Create an empty DJSets data structure
    DJSets();
    // Create a DJSets data structure with a specified number of elements (with element id's from 0 to count-1)
    DJSets(int count);
    // Copy constructor
    DJSets(const DJSets& s);
    // Destructor
    ~DJSets();
    ...
    // Combine two sets into one using the sets the elements belong to
    // All elements in those two sets will share the same set id that can be gotten using FindSet.
    void  UnionElementSets(int elementId1, int elementId2);
    ...
    // Add a specified number of elements to the DJSets data structure. The element id's of the new elements are numbered
    // consequitively starting with the first never-before-used elementId.
    void AddElements(int numToAdd);

    // Returns the number of elements currently in the DJSets data structure.
    int NumElements() const;

    // Returns the number of sets currently in the DJSets data structure.
    int NumSets() const;
...
private:
    ...
    int m_numElements; // the number of elements currently in the DJSets data structure.
    int m_numSets; // the number of sets currently in the DJSets data structure.
