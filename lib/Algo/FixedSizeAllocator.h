...
public: // methods
	...
	USER_TYPE *alloc()
	{
		...
		else
		{
			...
			if( pNewNode->pNext )
			{
				pNewNode->pNext->pPrev = 0;
			}

			// node is now on the used list

			pNewNode->pPrev = 0; // the allocated node is always first in the list

			if( m_pFirstUsed == 0 )
			{
				pNewNode->pNext = 0; // no other nodes
			}
			else
			{
