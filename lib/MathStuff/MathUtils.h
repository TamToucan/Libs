...
#define FLOATS_EQUAL(f1, f2)                                                   \
  (fabs(f2 - f1) <= std::numeric_limits<float>::epsilon() * fabs(f1))
...
inline float xchange(float heading) { return cosf(DEG_TO_RAD(heading + 90)); }

// Turn heading (0 = North) into Y Change (-1...1)
inline float ychange(float heading) { return sinf(DEG_TO_RAD(heading + 90)); }
